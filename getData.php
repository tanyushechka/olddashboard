...
for ($i = 1; $i <= 1; $i++) {          // для каждой активности делаем следующее:
    $currentAction = rand(0, 4);     // определяем тип активности
    $currentEmployeeId = rand(0, 30);    // определяем сотрудника
    $currentDuration = rand(300, 6000);  // длительность активности в секундах
    $currentCreatedAt = rand(300, 6000);  // начала активности - метка юникс-тайм
